...
interface MaterialInputProps {
  onSubmit: (title: string, content: string, sourceType?: string) => Promise<void>;
  isLoading?: boolean;
}
...
export function MaterialInput({ onSubmit, isLoading }: MaterialInputProps) {
  ...
  const [lastSourceType, setLastSourceType] = useState<string>("manual");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSubmit(title, content, lastSourceType);
    setTitle("");
    setContent("");
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const selectedModel = localStorage.getItem("ordknow_model") || "deepseek-chat";
      const ocrModel = selectedModel;
      ...
      if (ext === "txt") {
        ...
        if (res.ok) {
          ...
          if (data.text) {
            ...
            if (!title && data.suggestedTitle) setTitle(data.suggestedTitle);
            toast("文件解析完成", "success");
            setLastSourceType("imported_doc");
          }
        } else {
          toast("文件解析失败", "error");
        }
  ...
  const handleAudioUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.configured === false) {
        ...
        setLastSourceType("audio_transcript");
      } else {
